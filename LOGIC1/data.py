...
class Data:
    ...
    def parse_data(self, label, h, i, mecab):
        ...
        bmm = brand + ' ' + model+ ' ' + maker
        
        if six.PY3:
            product = product.decode('utf-8')
        
        product = product.upper()
        split_product = re_sc.sub(' ', product).strip().split()
        words = [w.strip() for w in split_product]
        ...
        bmm = bmm.lower()
        split_bmm = re_sc.sub(' ', bmm).strip().split()
        words2 = [w.strip() for w in split_bmm]
        words2 = [w for w in words2
                 if len(w) >= opt.min_word_length and len(w) < opt.max_word_length]
        ...
        split_product = UPPER.sub(' ', uni_product).strip()
        split_product = re_sc.sub(' ', split_product).strip().split()
        words3 = [w.strip() for w in split_product]
        words3 = [w for w in words3
                 if len(w) >= opt.min_word_length and len(w) < opt.max_word_length]
        group_jamo = []
        for w in words3:
            g_jamo =  hgtk.text.decompose(w)
            g_jamo = g_jamo.replace(u"ᴥ", "")
            group_jamo.append(g_jamo)

        strip_uni_product2 = LOWER.sub(' ', strip_uni_product).strip()
        strip_uni_product2 = UPPER.sub(' ', strip_uni_product2).strip()
        words4 = strip_uni_product2.replace(" ", "")
        group_jamo2 = []
        for w in words4:
            g_jamo2 =  hgtk.text.decompose(w)
            g_jamo2 = g_jamo2.replace(u"ᴥ", "")
            group_jamo2.append(g_jamo2)

        jamo =  hgtk.text.decompose(strip_uni_product)
        ...
        b = [hash_func(w) % opt.unigram_hash_size + 1 for w in words2]
        n = [hash_func(w) % opt.unigram_hash_size + 1 for w in ngram]

        l = [hash_func(w) % opt.unigram_hash_size + 1 for w in group_jamo]
        k = [hash_func(w) % opt.unigram_hash_size + 1 for w in group_jamo2]
        j = [hash_func(w) % opt.unigram_hash_size + 1 for w in jamo] 
 
        xv = Counter(x).most_common(opt.max_len)
        se = Counter(s).most_common(opt.max_len)
        NO = Counter(N).most_common(opt.max_len)
        
        bm = Counter(b).most_common(opt.max_len)
        ng = Counter(n).most_common(opt.ngram_max_len)

        lp = Counter(l).most_common(opt.max_len)
        ko = Counter(k).most_common(opt.max_len)
        ja = Counter(j).most_common(opt.max_len)
        ...
        N = np.zeros(opt.max_len, dtype=np.float32)
        O = np.zeros(opt.max_len, dtype=np.int32) 

        b = np.zeros(opt.max_len, dtype=np.float32)
        ...
        l = np.zeros(opt.max_len, dtype=np.float32)
        p = np.zeros(opt.max_len, dtype=np.int32)   
        k = np.zeros(opt.max_len, dtype=np.float32)
        o = np.zeros(opt.max_len, dtype=np.int32)   
        j = np.zeros(opt.max_len, dtype=np.float32)
        a = np.zeros(opt.max_len, dtype=np.int32)   
        ...
        return Y, (x, v, s, e, N, O, b, m, n, g, l, p, k, o, j, a, image)
    ...
    def create_dataset(self, g, size, num_classes):
        ...
        g.create_dataset('bmm', shape, chunks=True, dtype=np.int32)
        g.create_dataset('w_bmm', shape, chunks=True, dtype=np.float32)
        ...
        g.create_dataset('jamo_g', shape, chunks=True, dtype=np.int32)
        g.create_dataset('w_jamo_g', shape, chunks=True, dtype=np.float32)
        g.create_dataset('jamo_g2', shape, chunks=True, dtype=np.int32)
        g.create_dataset('w_jamo_g2', shape, chunks=True, dtype=np.float32)
        g.create_dataset('jamo', shape, chunks=True, dtype=np.int32)
        g.create_dataset('w_jamo', shape, chunks=True, dtype=np.float32)
    ...
    def init_chunk(self, chunk_size, num_classes):
        ...
        chunk['bmm'] = np.zeros(shape=chunk_shape, dtype=np.int32)
        chunk['w_bmm'] = np.zeros(shape=chunk_shape, dtype=np.float32)
        chunk['ngram'] = np.zeros(shape=ngram_shape, dtype=np.int32)
        chunk['w_ngram'] = np.zeros(shape=ngram_shape, dtype=np.float32)

        chunk['jamo_g'] = np.zeros(shape=chunk_shape, dtype=np.int32)
        chunk['w_jamo_g'] = np.zeros(shape=chunk_shape, dtype=np.float32)
        chunk['jamo_g2'] = np.zeros(shape=chunk_shape, dtype=np.int32)
        chunk['w_jamo_g2'] = np.zeros(shape=chunk_shape, dtype=np.float32)
        chunk['jamo'] = np.zeros(shape=chunk_shape, dtype=np.int32)
        chunk['w_jamo'] = np.zeros(shape=chunk_shape, dtype=np.float32)
    ...
    def copy_chunk(self, dataset, chunk, offset, with_pid_field=False):
        ...
        dataset['bmm'][offset:offset + num, :] = chunk['bmm'][:num]
        dataset['w_bmm'][offset:offset + num, :] = chunk['w_bmm'][:num]
        dataset['ngram'][offset:offset + num, :] = chunk['ngram'][:num]
        dataset['w_ngram'][offset:offset + num, :] = chunk['w_ngram'][:num]

        dataset['jamo_g'][offset:offset + num, :] = chunk['jamo_g'][:num]
        dataset['w_jamo_g'][offset:offset + num, :] = chunk['w_jamo_g'][:num]
        dataset['jamo_g2'][offset:offset + num, :] = chunk['jamo_g2'][:num]
        dataset['w_jamo_g2'][offset:offset + num, :] = chunk['w_jamo_g2'][:num]
        dataset['jamo'][offset:offset + num, :] = chunk['jamo'][:num]
        dataset['w_jamo'][offset:offset + num, :] = chunk['w_jamo'][:num]
    ...
    def copy_bulk(self, A, B, offset, y_offset, with_pid_field=False):
        ...
        A['bmm'][offset:offset + num, :] = B['bmm'][:num]
        A['w_bmm'][offset:offset + num, :] = B['w_bmm'][:num]
        A['ngram'][offset:offset + num, :] = B['ngram'][:num]
        A['w_ngram'][offset:offset + num, :] = B['w_ngram'][:num]

        A['jamo_g'][offset:offset + num, :] = B['jamo_g'][:num]
        A['w_jamo_g'][offset:offset + num, :] = B['w_jamo_g'][:num]
        A['jamo_g2'][offset:offset + num, :] = B['jamo_g2'][:num]
        A['w_jamo_g2'][offset:offset + num, :] = B['w_jamo_g2'][:num]
        A['jamo'][offset:offset + num, :] = B['jamo'][:num]
        A['w_jamo'][offset:offset + num, :] = B['w_jamo'][:num]
    ...
    def make_db(self, data_name, output_dir='data/train', train_ratio=0.8):
        ...
        for input_chunk_idx in chunk_order:
            ...
            for data_idx, (pid, y, vw) in data:
                if y is None:
                    continue
                v, w, s, e, N, O, b, m, n, g, l, p, k, o, j, a, i = vw
        ...
        for div in ['train', 'dev']:
            ...
            ds['bmm'].resize(shape)
            ds['w_bmm'].resize(shape)
            ds['ngram'].resize(ngram_shape)
            ds['w_ngram'].resize(ngram_shape)

            ds['jamo_g'].resize(shape)
            ds['w_jamo_g'].resize(shape)
            ds['jamo_g2'].resize(shape)
            ds['w_jamo_g2'].resize(shape)
            ds['jamo'].resize(shape)
            ds['w_jamo'].resize(shape)
